...
def visualize_point_cloud(point_cloud, name):
    ...
    fig.write_image(f'pcs/{name}.png')
    return fig

# Get list of .npy files in the directory
directory = './pcs'
files = sorted([f for f in os.listdir(directory) if f.endswith('.npy')])
names = np.arange(0, 100, 10)
names = [str(i) for i in names]

for selected_file, name in zip(files, names):
    file_path = os.path.join(directory, selected_file)
    point_cloud = load_point_cloud(file_path)
    fig = visualize_point_cloud(point_cloud, name)
