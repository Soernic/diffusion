...
def visualize_point_cloud(point_cloud, title, camera_scaling, height_adjustment):
    ...
    layout = go.Layout(
        width=300,  # Further reduce width
        height=250,  # Further reduce height
...
def create_and_save_grid(main_folder, output_file, camera_scaling=1.1, height_adjustment=-0.5):
    ...
    for subfolder in subfolders:
        for file in files:
            ...
            rotated_point_cloud = rotate_point_cloud(point_cloud, np.pi / 2)  # Rotate 90 degrees around the x-axis
    ...
    fig.update_layout(
        height=1500,  # Further adjust height to fit tighter
        width=900,  # Further adjust width to fit tighter
        showlegend=False
    )
