...
# Set CUDA_VISIBLE_DEVICES based on the parsed arguments
if args.parallel:
    os.environ["CUDA_VISIBLE_DEVICES"] = args.device_list

# Clear the unused cache to prevent GPU memory errors
...
# Wrap the model with DataParallel if parallel is True
...
def train(it):
    ...
    loss = model.module.get_loss(x, kl_weight=kl_weight, writer=writer, it=it) if args.parallel else model.get_loss(x, kl_weight=kl_weight, writer=writer, it=it)
...
def validate_inspect(it):
    ...
    x = model.module.sample(z, args.sample_num_points, flexibility=args.flexibility) if args.parallel else model.sample(z, args.sample_num_points, flexibility=args.flexibility)
